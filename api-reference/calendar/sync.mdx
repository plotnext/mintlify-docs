...
```ts
function inferChannelFromJoinUrl(url?: string | null): "gmeet" | "zoom" | "teams" | "offline" {
...
async function upsertInitialEvents(
  ...
  const meetingsApiUrl = process.env.MEETING_API_URL || "https://www.arali.xyz/api/v1/meetings";
  ...
  for (const ev of items) {
    if (!ev) continue;
    const isSeriesMaster = Array.isArray(ev.recurrence) && ev.recurrence.length > 0 && !ev.recurringEventId;
    if (isSeriesMaster) {
  ...
  for (const ev of seriesMasters) {
    const status: "cancelled" | "active" = ev.status === "cancelled" ? "cancelled" : "active";
    const dataToUpsert = {
  ...
  for (const ev of oneOffEvents) {
    ...
    const joinUrl = ev.hangoutLink || ev.conferenceData?.entryPoints?.find((p) => p.entryPointType === "video")?.uri || null;
